...
const UpdateProductModal = () => {
  const dispatch = useDispatch();
    ...
    const [formData, setFormData] = useState({name:product?.name, price:product?.price, description:product?.description, image:product?.image, category:product?.category, company:product?.company, inventory:product?.inventory, featured:product?.featured, freeShipping:product?.freeShipping})
    ...
    useEffect(() => {
      setFormData({name:product?.name, price:product?.price, description:product?.description, image:product?.image, category:product?.category, company:product?.company, inventory:product?.inventory, featured:product?.featured, freeShipping:product?.freeShipping});
    }, [product])
  ...
  return (
    <form onSubmit={handleSubmit} className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] z-10  bg-[#D9C6A7] rounded-xl p-3 w-[21rem]">
      ...
      <input type="name" name="name" id="name" required value={formData.name} onChange={handleChange} placeholder="Enter name..."
      ...
      <input type="number" name="price" id="price" required value={formData.price} onChange={handleChange} placeholder="Enter price..."
       className="bg-[#D9C6A7] text-[#0D1732] focus:outline-none border border-gray-300 rounded py-2 px-4 block w-full appearance-none placeholder-[#3A3B3C]"/>
      </div>
        <div className="flex flex-col gap-3">
      <label htmlFor="description" className="text-[#EEEDE8]">Description</label>
      <input type="text" name="description" id="description" required value={formData.description} onChange={handleChange} placeholder="Enter description..."
       className="bg-[#D9C6A7] text-[#0D1732] focus:outline-none border border-gray-300 rounded py-2 px-4 block w-full appearance-none placeholder-[#3A3B3C]"/>
      </div>
        <div className="flex flex-col gap-3">
      <label htmlFor="image" className="text-[#EEEDE8]">Image</label>
      <input type="text" name="image" id="image" required value={formData.image} onChange={handleChange} placeholder="Enter image..."
       className="bg-[#D9C6A7] text-[#0D1732] focus:outline-none border border-gray-300 rounded py-2 px-4 block w-full appearance-none placeholder-[#3A3B3C]"/>
      </div>
      <div className="flex flex-col gap-3">
...
<select
      ...
      <input type="number" name="inventory" id="inventory" required value={formData.inventory} onChange={handleChange} placeholder="Enter inventory..."
       className="bg-[#D9C6A7] text-[#0D1732] focus:outline-none border border-gray-300 rounded py-2 px-4 block w-full appearance-none placeholder-[#3A3B3C]"/>
      </div>
      </section>

      <button type="submit" className="btn btn-neutral">Update Product</button>
      </div>
    </form>
  )
}
